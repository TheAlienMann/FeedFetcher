//
//  FeedFetcherApiEndToEndTests.swift
//
...
final class FeedFetcherApiEndToEndTests: XCTestCase {
  ...
  func test_endToEndTestServerGETFeedResult_matchesFixedTestAccountData() {
    ...
    switch receivedResult {
      case let .success(imageFeed):
        XCTAssertEqual(imageFeed.count, 8, "Expected 8 feed images from the testing server.")
  ...
  func test_endToEndTestServerGETFeedImage_matchesFixedTestAccountData() {
    ...
    switch result {
      ...
      default:
        XCTFail("Expected not empty image data result but got result.")
    }
  }
  ...
  private func getFeedResult(file: StaticString = #file, line: UInt = #line) -> RemoteFeedLoader.Result? {
    ...
    let loader = RemoteFeedLoader(from: testingServerURL, httClient: client)
  ...
  func getLoadFeedImageResult(file: StaticString = #file, line: UInt = #line) -> RemoteFeedImageLoader.Result? {
    ...
    trackForMemoryLeak(imageLoader, file: file, line: line)
    
    let exp = expectation(description: "Waiting for load image data to complete.")
    var receivedResult: RemoteFeedImageLoader.Result?
    let _ = imageLoader.loadImageData(from: testingServerURl, completion: { result in
      receivedResult = result
  ...
  func httpClientWithEphemeralSession(file: StaticString = #file, line: UInt = #line) -> HTTPClient {
