...
class CoreDataFeedImageDataStoreTests: XCTestCase {
  ...
  func test_sideEffects_runSerially() {
    ...
    sut.insertImageData(anyData(), for: url, completion: { _ in op2.fulfill() })
    
    let op3 = expectation(description: "Operation 3")
    sut.insertImageData(anyData(), for: url, completion: { _ in op3.fulfill() })
  ...
  func expect(_ sut: CoreDataFeedStore, toCompleteRetrievalWith expected: FeedImageDataStore.RetrieveResult, for url: URL, file: StaticString = #file, line: UInt = #line) {
    ...
    sut.retrieveImageData(for: url, completion: { result in
      switch (expected, result) {
        ...
        default:
          XCTFail("Expected to complete with not found data for url that is not stored, but got \(result) instead.", file: file, line: line)
      }
      exp.fulfill()
    })
  ...
  func insert(_ data: Data, for url: URL, into sut: CoreDataFeedStore, file: StaticString = #file, line: UInt = #line) {
    ...
    sut.insert([image], timestamp: Date(), completion: { insertFeedResult in
      switch insertFeedResult {
        case let .failure(insertFeedImageError):
          ...
          exp.fulfill()
        case .success:
          sut.insertImageData(data, for: url, completion: { insertDataResult in
